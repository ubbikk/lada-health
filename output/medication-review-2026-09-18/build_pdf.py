...
styles={
 ...
 'h':ParagraphStyle('h',fontName='DejaVu-Bold',fontSize=13,leading=18,spaceBefore=9,spaceAfter=6),
...
story=[]
story += [P('Ліки Лади','title'),P('<b>Щоденний розклад • Початок 14 вересня 2026 року</b>'),P('Розклад для спрощеної схеми. Час можна змістити під сон і прийоми їжі, зберігаючи інтервали. Підйом о 12:00; сніданок о 13:00, вечеря о 23:00. Останній прийом - о 01:00 наступної календарної доби.'),P('<b>Оновлено 18 вересня 2026 року:</b> до ранкового прийому додано вітамін B1 (NOW).')]
story += [T([
...
story += [PageBreak(),P('Строки та важливі примітки','title'),P('Курси ліків відраховано від <b>14.09.2026</b>; для <b>NOW B-1</b> - від першого прийому. Останній зазначений день входить у курс. Прийом о 01:00 належить до попереднього дня розкладу; остання нічна доза - у ніч після зазначеної дати. Після завершення курсу прибрати відповідний рядок зі щоденного розкладу, враховуючи винятки нижче.')]
...
def footer(c,d):
    c.setFont('DejaVu',8);c.drawString(38,22,'Лада • Оновлено 18.09.2026');c.drawRightString(A4[0]-38,22,str(d.page))
...
(root/'output/medication-review-2026-09-18/simplified-pdf-text.txt').write_text(text)
